...
class WorkUnit:
    def __init__(self, work_id, environment, num_experiments, num_episodes, hyperparams, checkpoint_Q=None):
...
class ExperimentalRun:
    def __init__(self, run_id, work_unit, checkpoint_frequency):
        ...
        self.results = [None]*len(work_units)
        self.remaining = [work_unit.num_experiments] * len(work_units)
        self.reserved = [0]*len(work_units)
    ...
    def get_next(self, num_cores):
        ...
        if next_remaining == -1:
            return None, 0
        if self.reserved[next_remaining] >= self.remaining[next_remaining]:
            self.reserved[next_remaining] = 0
        wu = self.work_units[next_remaining]
        ret_wu = WorkUnit(wu.work_id, wu.environment, min(num_cores, self.remaining[next_remaining] - self.reserved[next_remaining]), wu.num_episodes, wu.hyperparams)
        if next_remaining >= 1:
            f = io.BytesIO()
            np.savez_compressed(f, **{"data": self.results[next_remaining - 1][1]})
            f.seek(0)
            ret_wu.checkpoint_Q = base64.b64encode(f.read()).decode("utf-8")
        retval = ret_wu, self.remaining[next_remaining] - self.reserved[next_remaining] - ret_wu.num_experiments
        self.reserved[next_remaining] += ret_wu.num_experiments
        return retval
        
    def requires_work(self, work_id):
        assert work_id[0] == self.run_id
        return self.remaining[work_id[1]] > 0
    def partial_complete(self, work_id, rewards, q_tables, test_rewards):
        ...
        if self.results[work_id[1]] is None:
            self.results[work_id[1]] = [relevant_rewards, relevant_q_tables, relevant_test_rewards]
        else:
            x = self.results[work_id[1]]
            x[0] = np.concatenate([x[0], relevant_rewards], axis=0)
            x[1] = np.concatenate([x[1], relevant_q_tables], axis=0)
            x[2] = np.concatenate([x[2], relevant_test_rewards], axis=0)
        self.remaining[work_id[1]] -= relevant_rewards.shape[0]
        if work_id[1] >= 1 and self.remaining[work_id[1]] == 0:
            self.results[work_id[1] - 1][1] = None # Delete previous qtables to save memory
    def is_complete(self):
        return all([x == 0 for x in self.remaining])
    def gather_results(self):
        ...
        rewards_dict = {}
        q_tables_dict = {}
        rewards_dict[key] = np.concatenate([x for x, _, _ in self.results], axis=1)
        test_rewards_dict[key] = np.concatenate([z for _, _, z in self.results], axis=1)
        q_tables_dict[key] = self.results[-1][1]
    ...
    def split(self, work_unit, num_episodes_per_unit):
        ...
        while total_episodes > 0:
            ...
            unit = WorkUnit(work_id, work_unit.environment, work_unit.num_experiments, [num_train_episodes, num_test_episodes], work_unit.hyperparams)
...
def get_work():
    ...
    while num_cores_left > 0 and num_experiments > 0:
        ...
        i += 1
        num_experiments -= 1
        num_cores_left -= wu.num_experiments
...
def work_status():
    return jsonify({"queue": [{"id": experiment.run_id, "remaining": experiment.remaining, "complete": experiment.is_complete()} for experiment in experiment_queue]})
...
def complete_work():
    ...
    for work_id in assignment_id:
        experiments[work_id[0]].partial_complete(work_id, rewards, q_tables, test_rewards)
    return "Success", 200
discretization = script.MC_DISCRETIZATION
env = script.MC_ENV_NAME
num_experiments = 5
num_episodes = [2000, 100]
runs = [{"agent": agent, "alpha": alpha, "gamma": gamma, "init_Q": init_Q, "temperature": temperature, "epsilon": None}
        for agent in ["BSC", "CSC", "RSC", "BEA", "CEA", "REA"]
        for alpha in [0.01, 0.1]
        for gamma in [0.99, 0.999]
        for init_Q in [-50]
        for temperature in [1]]
work_units = [WorkUnit(None, env, num_experiments, num_episodes, run) for run in runs]
experiment_queue = [ExperimentalRun(i, wu, 250) for i, wu in enumerate(work_units)]
